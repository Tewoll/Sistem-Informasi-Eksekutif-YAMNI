	...
	if ($level != 'admin') {
		...
		switch($dct) 
		{
			...
			if($nama == NULL){
				...
				echo '<meta http-equiv="refresh" content="2;url=index.php?page=siswa_ma">';
			}else{
				...
				if ($qsiswa == true) {
					...
					Swal.fire({
						...
						window.setTimeout(function(){ 
							window.location.replace('index.php?page=siswa_ma');
							} ,3000); 
							</script>";
				}else{
					// include_once 'switch/pesan-gagal.php';
					...
					Swal.fire({
						...
						window.setTimeout(function(){ 
							window.location.replace('index.php?page=siswa_ma&act=edit&id=$id');
							} ,3000); 
							</script>";
				}
			}
			...
			case 'import':
				...
				$target = basename($_FILES['filesiswa_ma']['name']) ;
				move_uploaded_file($_FILES['filesiswa_ma']['tmp_name'], $target);

				// beri permisi agar file xls dapat di baca
				chmod($_FILES['filesiswa_ma']['name'],0777);

				// mengambil isi file xls
				$data = new Spreadsheet_Excel_Reader($_FILES['filesiswa_ma']['name'],false);
				...
				if ($jenis_tingkat == 'Sekolah Menengah Pertama (SMP)') {
					$id_jt = "7";
				}elseif($jenis_tingkat == 'Madrasah Tsanawiyah (MTs)'){
					$id_jt = "2";
				}else {
					$id_jt = "5";
				}
				...
				if($nama != "" && $jenkel != "" && $tempat_lahir != "" && $tgl_lahir != "" && $nama_ortu != "" && $id_tingkat != "" && $kelas != "" && $alamat != "" && $kota !== "" && $status_ekonomi != ""){
				...
				unlink($_FILES['filesiswa_ma']['name']);
				echo "<script type='text/javascript'>
				Swal.fire({
					position: 'center',
					icon: 'success',
			...
			default:
				echo '<meta http-equiv="refresh" content="0;url=index.php?page=siswa_ma">';
			break;
		}
		?>
<?php }} ?>