...
switch($dct) 
{
	case 'tambah':
			...
			if($nama == NULL){
				...
				echo '<meta http-equiv="refresh" content="2;url=index.php?page=siswa_mts">';
			}else{
				...
				if ($qsiswa == true) {
					...
					Swal.fire({
						...
						window.setTimeout(function(){ 
							window.location.replace('index.php?page=siswa_mts');
							} ,3000); 
							</script>";
				}else{
					// include_once 'switch/pesan-gagal.php';
					...
					Swal.fire({
						...
						window.setTimeout(function(){ 
							window.location.replace('index.php?page=siswa_mts&act=edit&id=$id');
							} ,3000); 
							</script>";
				}
			}
	...
	case 'import':
		...
		$target = basename($_FILES['filesiswa_mts']['name']) ;
		move_uploaded_file($_FILES['filesiswa_mts']['tmp_name'], $target);

		// beri permisi agar file xls dapat di baca
		chmod($_FILES['filesiswa_mts']['name'],0777); 

		// mengambil isi file xls
		$data = new Spreadsheet_Excel_Reader($_FILES['filesiswa_mts']['name'],false);
		...
		if ($jenis_tingkat == 'Sekolah Dasar (SD)') {
			$id_jt 			= "6";
		} elseif($jenis_tingkat == 'Madrasah Ibtidaiah (MI)'){
			$id_jt 			= "3";
		} else {
			$id_jt 			= "5";
		}
		...
		unlink($_FILES['filesiswa_mts']['name']);
		echo "<script type='text/javascript'>
		Swal.fire({
			position: 'center',
			icon: 'success',
	...
	default:
		echo '<meta http-equiv="refresh" content="0;url=index.php?page=siswa_mts">';
	break;
}
?>
<?php } }?>