...
switch($dct) 
{
			...
			if (empty($lokasi_file)) {
				...
				echo "<script type='text/javascript'>
						Swal.fire({
							...
							window.setTimeout(function(){ 
								window.location.replace('index.php?page=guru_mts');
								} ,3000); 
								</script>";
			}else{
				if (!in_array($ext,$ekstensi) ) {
					echo "<script type='text/javascript'>
						Swal.fire({
							...
							window.setTimeout(function(){ 
								window.location.replace('index.php?page=guru_mts&act=tambah);
								} ,3000); 
								</script>";
				}else{
	...
	case 'ubah':
		if (isset($_POST['simpan'])) {
			...
			if($cekdata > 0){
				if (empty($lokasi_file)){
					$qguru = mysqli_query($connection,"UPDATE guru SET nama_guru = '{$nama}', jenkel_guru = '{$jenkel}', id_pendidikan = '{$id_pendidikan}', tmpt_lahir = '{$tmpt_lahir}', tgl_lahir = '{$tgl_lahir}', id_jabatan = '{$id_jabatan}', tgl_mulai = '{$tgl_mulai}', id_tingkat= '{$id_tingkat}', pns_nonpns = '{$pns}', alamat = '{$alamat}' WHERE id = '{$id}'") OR die('Ada kesalahan pada query '.mysqli_error($connection));
					if ($qguru == true) {
						/* untuk alamat */
						echo "<script type='text/javascript'>
						Swal.fire({
						...
						Swal.fire({
							...
							window.setTimeout(function(){ 
								window.location.replace('index.php?page=guru_mts&act=edit&id=$id');
								} ,3000); 
								</script>";
					}
				}else { 
					$foto_lama = inject_checker($connection,$_POST['foto_lama']);
					$qguru = mysqli_query($connection,"UPDATE guru SET nama_guru = '{$nama}', jenkel_guru = '{$jenkel}', id_pendidikan = '{$id_pendidikan}', tmpt_lahir = '{$tmpt_lahir}', tgl_lahir = '{$tgl_lahir}', id_jabatan = '{$id_jabatan}', tgl_mulai = '{$tgl_mulai}', id_tingkat= '{$id_tingkat}', pns_nonpns = '{$pns}', alamat = '{$alamat}', foto = '{$nama_gambar}' WHERE id = '{$id}'") OR die('Ada kesalahan pada query '.mysqli_error($connection));
					if ($qguru == true) {
	...
	case 'import':
		...
		$target = basename($_FILES['fileguru_mts']['name']) ;
		move_uploaded_file($_FILES['fileguru_mts']['tmp_name'], $target);

		// beri permisi agar file xls dapat di baca
		chmod($_FILES['fileguru_mts']['name'],0777);

		// mengambil isi file xls
		$data = new Spreadsheet_Excel_Reader($_FILES['fileguru_mts']['name'],false);
		...
		unlink($_FILES['fileguru_mts']['name']);
		echo "<script type='text/javascript'>
		Swal.fire({
			position: 'center',
			icon: 'success',
	...
	default:
		echo '<meta http-equiv="refresh" content="0;url=index.php?page=guru_mts">';
	break;
}
?>
<?php } } ?>