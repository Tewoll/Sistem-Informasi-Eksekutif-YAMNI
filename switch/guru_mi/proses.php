...
switch($dct) 
{
			...
			if (empty($lokasi_file)) {
				...
				echo "<script type='text/javascript'>
						Swal.fire({
							...
							window.setTimeout(function(){ 
								window.location.replace('index.php?page=guru_mi');
								} ,3000); 
								</script>";
			}else{
				if (!in_array($ext,$ekstensi) ) {
					echo "<script type='text/javascript'>
						Swal.fire({
							...
							window.setTimeout(function(){ 
								window.location.replace('index.php?page=guru_mi&act=tambah);
								} ,3000); 
								</script>";
				}else{
				...
				if (empty($lokasi_file)){
					...
					if ($qguru == true) {
						...
						Swal.fire({
							...
							window.setTimeout(function(){ 
								window.location.replace('index.php?page=guru_mi&act=edit&id=$id');
								} ,3000); 
								</script>";
					}
				}else { 
	...
	case 'import':
		...
		$target = basename($_FILES['fileguru_mi']['name']) ;
		move_uploaded_file($_FILES['fileguru_mi']['tmp_name'], $target);

		// beri permisi agar file xls dapat di baca
		chmod($_FILES['fileguru_mi']['name'],0777);

		// mengambil isi file xls
		$data = new Spreadsheet_Excel_Reader($_FILES['fileguru_mi']['name'],false);
		...
		unlink($_FILES['fileguru_mi']['name']);
		echo "<script type='text/javascript'>
		Swal.fire({
			position: 'center',
			icon: 'success',
	...
	break;
	default:
		echo '<meta http-equiv="refresh" content="0;url=index.php?page=guru_mi">';
	break;
}
?>
<?php }} ?>